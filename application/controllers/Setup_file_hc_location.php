...
class Setup_file_hc_location extends Root_Controller
{
    ...
    private function system_list()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $method='list';
            $data['system_preference_items']= System_helper::get_preference($user->user_id,$this->controller_url,$method,$this->get_preference_headers($method));
            $data['title']="Hard Copy Location Active List";
            $ajax['status']=true;
    ...
    {
        $this->db->from($this->config->item('table_fms_setup_file_hc_location').' item');
    ...
    private function system_list_all()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $method='list_all';
            $data['system_preference_items']= System_helper::get_preference($user->user_id,$this->controller_url,$method,$this->get_preference_headers($method));
            $data['title']="Hard Copy Location All List";
            $ajax['status']=true;
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['action2'])&&($this->permissions['action2']==1))
        {
            ...
            $data['title']="Edit Hard Copy Location :: ". $data['item']['name'];
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view($this->controller_url."/add_edit",$data,true));
            if($this->message)
            {
    ...
    private function system_save()
    {
        ...
        if($id>0)
        {
            ...
            Query_helper::update($this->config->item('table_fms_setup_file_hc_location'),$item,array('id='.$id));
        }
        else
        {
            $item['date_created']=$time;
            $item['user_created']=$user->user_id;
            Query_helper::add($this->config->item('table_fms_setup_file_hc_location'),$item);
        }
