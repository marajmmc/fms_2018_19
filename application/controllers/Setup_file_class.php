...
class Setup_file_class extends Root_Controller
{
    ...
    private function system_list()
    {
        if (isset($this->permissions['action0']) && ($this->permissions['action0'] == 1))
        {
            ...
            $method = 'list';
            $data['system_preference_items'] = System_helper::get_preference($user->user_id, $this->controller_url, $method, $this->get_preference_headers($method));
            $data['title'] = "Active File Class List";
            $ajax['status'] = true;
    ...
    private function system_list_all()
    {
        if (isset($this->permissions['action0']) && ($this->permissions['action0'] == 1))
        {
            ...
            $method = 'list_all';
            $data['system_preference_items'] = System_helper::get_preference($user->user_id, $this->controller_url, $method, $this->get_preference_headers($method));
            $data['title'] = "All File Class List";
            $ajax['status'] = true;
    ...
    private function system_save()
    {
        ...
        if ($id > 0) // EDIT
        {
            ...
            $result = Query_helper::get_info($this->config->item('table_fms_setup_file_class'), '*', array('id =' . $id, 'status != "' . $this->config->item('system_status_delete') . '"'), 1);
            if (!$result)
            {
        ...
        if ($id > 0) // EDIT
        {
            ...
            Query_helper::update($this->config->item('table_fms_setup_file_class'), $item, array('id=' . $id));
        }
        else // ADD
        {
            $item['date_created'] = $time;
            $item['user_created'] = $user->user_id;
            Query_helper::add($this->config->item('table_fms_setup_file_class'), $item);
        }
    ...
    private function check_validation()
    {
        ...
        $this->form_validation->set_rules('item[id_sub_category]', $this->lang->line('LABEL_SUB_CATEGORY_NAME'), 'required');
        $this->form_validation->set_rules('item[name]', $this->lang->line('LABEL_CLASS_NAME'), 'required|trim');
